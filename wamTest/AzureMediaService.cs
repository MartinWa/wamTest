...
using Zero.Common.Enums;

namespace Zero.Data.AzureMediaService
{
    public class AzureMediaService : IMediaService
    {
        private readonly IZeroStorage _storage;
        private readonly IZeroSettings _settings;
        private readonly Lazy<CloudMediaContext> _cloudMediaContext;
        private const string EncodedFileExtensionn = ".mp4";

        public AzureMediaService(IZeroSettings settings, IZeroStorage storage, AzureMediaServiceFactory factory)
        {
            _settings = settings;
            _storage = storage;
        ...
        public async Task<IJob> CreateEncodeJobAsync(IZeroBlob original, string encodedFileName, string jobIdentifier, CancellationToken cancellationToken)
        {
            ...
            var assetContainer = _storage.GetContiner((new Uri(destinationLocator.Path)).Segments[1]);
        ...
        public async Task FinishEncodeJobAsync(string jobIdentifier, int contentId, string newFilename, CancellationToken cancellationToken)
        {
            ...
            if (outputAsset != null)
            {
                ...
                var encodedassetContainer = _storage.GetContiner((new Uri(encodedLocator.Path)).Segments[1]);
                // ReSharper disable once ReplaceWithSingleCallToFirstOrDefault Direct FirstOrDefault not supported
        ...
        public async Task<MediaEncodeProgressDto> GetEncodeProgressAsync(string jobIdentifier, IZeroBlob resultingFile)
        {
            // ReSharper disable once ReplaceWithSingleCallToFirstOrDefault Direct FirstOrDefault not supported
